...
| N26 | **Exportar los QR por flota** para enviarlos a imprimir (etiquetas). Botón "Imprimir QRs" en el detalle de flota → grilla imprimible de todos los vehículos con QR (`/flota/flotas/:codigo/qr`). Además se corrigió que **Imprimir QR de un vehículo mostraba página en blanco** (la ruta se servía como pública sin providers → crash de useVehiculos; ahora es interna) | 2026-08-04 | implementado |
