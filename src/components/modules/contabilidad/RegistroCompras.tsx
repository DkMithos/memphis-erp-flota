/**
 * Memphis ERP — Registro de Compras (PLE 8.1)
 * Lista de comprobantes recibidos por período con exportación PLE SUNAT.
 */
import { useState, useMemo } from 'react';
import { FileText, Download, Filter, TrendingDown } from 'lucide-react';
...
import { generarLineaPLE81, periodoToString } from '../../../lib/contabilidad/fiscal-peru';
...
export function RegistroCompras({ onNavigate }: Props) {
  ...
  const comprasPeriodo = useMemo(() => {
    ...
    return comprobantes.filter(c => {
      if (c.direccion !== 'recibido' || c.estado === 'anulado') return false;
  ...
  const totales = useMemo(() => ({
    baseGravada: comprasPeriodo.reduce((s,c) => s + c.opGravada, 0),
    igv: comprasPeriodo.reduce((s,c) => s + c.igv, 0),
    total: comprasPeriodo.reduce((s,c) => s + c.total, 0),
  }), [comprasPeriodo]);

  function exportarPLE() {
    const lineas = comprasPeriodo.map((c, i) => generarLineaPLE81({
      periodo,
      correlativo: String(i + 1),
      fechaEmision: c.fechaEmision,
      fechaVencimiento: c.fechaVencimiento ?? c.fechaEmision,
      tipoComprobante: c.tipo,
      serie: c.serie,
      anio: new Date(c.fechaEmision).getFullYear(),
      numero: c.numero,
      tipoDocIdentidad: '6',
      numDocIdentidad: c.rucEmisor ?? '',
      razonSocial: c.razonSocialEmisor ?? '',
      baseImponibleGravada: c.opGravada,
      igv: c.igv,
      baseImponibleNoGravada: c.opExonerada + c.opInafecta,
      importeTotal: c.total,
      moneda: c.moneda,
      tipoCambio: c.tipoCambio,
      indicadorPagoBancarizado: '0',
    ...
    a.href = url;
    a.download = `LE${periodo}00080100001100_RC.txt`;
    a.click();
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            <TrendingDown className="size-5 text-primary" />Registro de Compras
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">PLE 8.1 — SUNAT</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'IGV Crédito Fiscal', value: totales.igv, color: 'text-red-600' },
          { label: 'Total Compras', value: totales.total, color: 'text-foreground' },
        ].map(k => (
          <Card key={k.label}>
        ...
        <CardContent className="p-0">
          {!periodo ? (
            ...
            <div className="py-10 text-center space-y-2">
              ...
              <p className="text-sm text-muted-foreground">Sin comprobantes de compra en este período.</p>
            ...
            <>
              ...
              {comprasPeriodo.map((c, i) => (
                <div key={c.id} className="grid grid-cols-12 gap-1 px-4 py-2.5 border-b border-border/40 hover:bg-muted/20 text-sm items-center">
                  ...
                  <div className="col-span-12 md:col-span-3 truncate text-xs">
                    <span className="font-medium">{c.razonSocialEmisor ?? '—'}</span>
                    {c.rucEmisor && <span className="text-muted-foreground ml-1 font-mono text-[10px]">{c.rucEmisor}</span>}
                  </div>
                  <div className="col-span-4 md:col-span-1 text-right font-mono text-xs">{fmt(c.opGravada)}</div>
                  <div className="col-span-4 md:col-span-1 text-right font-mono text-xs text-red-600">{fmt(c.igv)}</div>
              ...
              <div className="grid grid-cols-12 gap-1 px-4 py-3 bg-muted/30 font-bold text-sm">
                ...
                <div className="col-span-1 text-right font-mono text-xs text-red-600">{fmt(totales.igv)}</div>
